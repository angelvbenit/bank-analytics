...
-- KPI 1: TOTAL CREDIT
DROP TABLE IF EXISTS total_credit;
CREATE TABLE total_credit (total_credit DECIMAL(18,4));
INSERT INTO total_credit
SELECT SUM(amount) FROM debitcredit WHERE transaction_type = 'Credit';
SELECT * FROM total_credit;

-- KPI 2: TOTAL DEBIT 
 DROP TABLE IF EXISTS total_debit;
CREATE TABLE total_debit (total_debit DECIMAL(18,4));
INSERT INTO total_debit
...
DROP TABLE IF EXISTS credit_debit_ratio;
CREATE TABLE credit_debit_ratio (credit_to_debit_ratio DECIMAL(10,4));
INSERT INTO credit_debit_ratio
SELECT ROUND(
    SUM(CASE WHEN transaction_type = 'Credit' THEN amount ELSE 0 END) /
    NULLIF(SUM(CASE WHEN transaction_type = 'Debit' THEN amount ELSE 0 END), 0),
4) FROM debitcredit;
SELECT * FROM credit_debit_ratio;

-- KPI 4: NET TRANSACTION AMOUNT 
DROP TABLE IF EXISTS net_transaction_amount;
CREATE TABLE net_transaction_amount (net_transaction_amount DECIMAL(18,4));
INSERT INTO net_transaction_amount
...
DROP TABLE IF EXISTS account_activity_ratio;
CREATE TABLE account_activity_ratio (overall_account_activity_ratio DECIMAL(18,6));
INSERT INTO account_activity_ratio
SELECT COUNT(*) / NULLIF(AVG(balance), 0) FROM debitcredit;
SELECT * FROM account_activity_ratio;

-- KPI 6a: TRANSACTIONS PER DAY
DROP TABLE IF EXISTS txn_per_day;
CREATE TABLE txn_per_day (txn_day DATE, total_transactions INT);
INSERT INTO txn_per_day
SELECT DATE(DATE_ADD('1899-12-30', INTERVAL transaction_date DAY)), COUNT(*)
FROM debitcredit GROUP BY 1 ORDER BY 1;
SELECT * FROM txn_per_day;

-- KPI 6b: TRANSACTIONS PER WEEK
DROP TABLE IF EXISTS txn_per_week;
CREATE TABLE txn_per_week (txn_week VARCHAR(20), total_transactions INT);
INSERT INTO txn_per_week
SELECT DATE_FORMAT(DATE_ADD('1899-12-30', INTERVAL transaction_date DAY), '%Y - Week %u'), COUNT(*)
FROM debitcredit GROUP BY 1 ORDER BY 1;
SELECT * FROM txn_per_week;

-- KPI 6c: TRANSACTIONS PER MONTH
DROP TABLE IF EXISTS txn_per_month;
CREATE TABLE txn_per_month (txn_month VARCHAR(10), total_transactions INT);
INSERT INTO txn_per_month
SELECT DATE_FORMAT(DATE_ADD('1899-12-30', INTERVAL transaction_date DAY), '%Y-%m'), COUNT(*)
FROM debitcredit GROUP BY 1 ORDER BY 1;
SELECT * FROM txn_per_month;


-- KPI 7: TOTAL TRANSACTION AMOUNT BY BRANCH 
DROP TABLE IF EXISTS amount_by_branch;
CREATE TABLE amount_by_branch (branch VARCHAR(100), total_amount DECIMAL(18,4));
INSERT INTO amount_by_branch
SELECT branch, SUM(amount) FROM debitcredit GROUP BY branch ORDER BY 2 DESC;
SELECT * FROM amount_by_branch;

-- KPI 8: TRANSACTION VOLUME BY BANK 
DROP TABLE IF EXISTS volume_by_bank;
CREATE TABLE volume_by_bank (bank_name VARCHAR(100), total_volume DECIMAL(18,4));
INSERT INTO volume_by_bank
SELECT bank_name, SUM(amount) FROM debitcredit GROUP BY bank_name ORDER BY 2 DESC;
SELECT * FROM volume_by_bank;

-- KPI 9: TRANSACTION METHOD DISTRIBUTION 
DROP TABLE IF EXISTS method_distribution;
CREATE TABLE method_distribution (transaction_method VARCHAR(100), transaction_count INT, percentage_distribution DECIMAL(6,2));
INSERT INTO method_distribution
...
DROP TABLE IF EXISTS branch_mom_growth;
CREATE TABLE branch_mom_growth (branch VARCHAR(100), txn_month VARCHAR(10), monthly_volume DECIMAL(18,4), prev_month_volume DECIMAL(18,4), mom_growth_pct DECIMAL(10,2));
INSERT INTO branch_mom_growth
...
CREATE TABLE total_transactions (total_transactions INT);
